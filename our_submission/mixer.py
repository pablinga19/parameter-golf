...
class PPMDMixer:
    def __init__(self, max_order=7, min_order=2, num_buckets=4_194_304,
                 min_count=2, primes=None, use_singleton_escape=True):
        if primes is None:
        ...
        self.singleton_tables = [np.zeros(num_buckets, dtype=np.uint16) for _ in range(self.n_orders)]
    ...
    def predict_blended(self, val_np, global_j, n_seg):
        ...
        for oi in range(self.n_orders):
            ...
            if got.any():
                ...
                if self.use_singleton_escape:
                    q1 = self.singleton_tables[oi][ck[got]].astype(np.float64)
                    esc = q1 / (2.0 * cc[got] + 1e-10)
                else:
                    uc = self.unique_tables[oi][ck[got]].astype(np.float64)
                    esc = uc / (cc[got] + uc + 1e-10)
    ...
    def update_tables(self, val_np, start, end):
        for j in range(start, end):
            ...
            for oi in range(self.n_orders):
                ...
                fc = self.full_tables[oi][fk]
                if fc == 0:
                    self.unique_tables[oi][ck] = min(65535, self.unique_tables[oi][ck] + 1)
                    self.singleton_tables[oi][ck] = min(65535, self.singleton_tables[oi][ck] + 1)
                elif fc == 1:
                    if self.singleton_tables[oi][ck] > 0:
                        self.singleton_tables[oi][ck] -= 1
